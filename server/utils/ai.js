...
export async function reconstructVisual(entityName, description, seedImage) {
  if (!ai) throw new Error("AI not configured");

  let parts = [
    { 
      text: `
        FORENSIC VISUAL RECONSTRUCTION: "${entityName}".
        
        STYLE SPECIFICATIONS:
        - Genuine amateur photography. 
        - Natural, somewhat harsh or incidental lighting (not studio lit).
        - Minor photographic flaws: slight motion blur, sensor noise, or slight overexposure.
        - The subject should look like a real person captured in a candid moment or a standard identification photo.
        - Highly realistic, no "AI polish" or plastic skin textures.
        - Indistinguishable from a genuine amateur photograph.
        
        CONTEXT: ${description}
        ${seedImage ? 'MANDATORY: Preserve the identity, facial structure, and physical characteristics of the person in the provided reference image. The output must look like the SAME PERSON but in the context described above.' : ''}
      `
    }
  ];
  ...
  try {
    ...
    console.error("Visual reconstruction failed:", e);
    throw e;
  }
}
